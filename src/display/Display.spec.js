...
describe("<Display />", () => {
  ...
  it("displays 'Closed' if the closed prop is true and 'Open' if otherwise. Part 1", () => {
    const { getByText } = render(<Display closed={true} />);
    const close = getByText(/closed/i).textContent;
    expect(close).toBeTruthy();
  });

  it("displays 'Closed' if the closed prop is true and 'Open' if otherwise. Part 2", () => {
    const { getByText } = render(<Display closed={false} />);
    const close = getByText(/open/i).textContent;
    expect(close).toBeTruthy();
  });

  it("displays 'Locked' if the locked prop is true and 'Unlocked' if otherwise. Part 1", () => {
    const { getByText } = render(<Display locked={true} />);
    const lock = getByText(/locked/i).textContent;
    expect(lock).toBeTruthy();
  });

  it("displays 'Locked' if the locked prop is true and 'Unlocked' if otherwise. Part 2", () => {
